...
#### Publishing to a separate `three-geospatial-webgpu` repository

To mirror the upstream setup (where the WebGPU Storybook is hosted from a dedicated repository such as `three-geospatial-webgpu`), supply the workflow with the repository information and a personal access token (PAT) with `repo` scope:

1. Create a classic PAT with `repo` access and add it as a secret named `WEBGPU_PAGES_TOKEN` (Settings → Secrets and variables → Actions).
2. Add a repository variable named `WEBGPU_PAGES_REPO` with the value `<your-user>/three-geospatial-webgpu`.
3. (Optional) If the target repository should be served from a different base path, set a repository variable `WEBGPU_PAGES_BASE_PATH` to that path (without leading/trailing slashes). By default the workflow uses the current repository name.
4. Run the **Deploy WebGPU Storybook** workflow. In addition to publishing to this repository’s Pages site, the workflow pushes the same static bundle to the `gh-pages` branch of the external repository via [`peaceiris/actions-gh-pages`](https://github.com/peaceiris/actions-gh-pages).

The next push to `main` (or manual workflow dispatch) will refresh both the `https://<your-user>.github.io/<this-repo>/` site and the external `https://<your-user>.github.io/three-geospatial-webgpu/` site.
